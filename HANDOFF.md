...
- **Voice commands during speech mode** — when speech-to-text is active, recognize command keywords and execute them instead of typing. E.g. "down" → down arrow, "up" → up arrow, "left" → left arrow, "right" → right arrow, "delete" → backspace, "enter" → return, "tab" → tab, "command tab" → Cmd+Tab, "escape" → escape, "select all" → Cmd+A, "undo" → Cmd+Z, "copy" → Cmd+C, "paste" → Cmd+V. Could use a prefix like "command" or "press" to distinguish from typing the word literally (e.g. "press down" vs typing "down"). This would solve the directional navigation problem that gesture-based pointing couldn't reliably handle.
- **Dynamic Island visual redesign** — current pill/toggle style looks cheap and out of place. Needs a proper design brainstorm — reference Apple's actual Dynamic Island (iPhone 14+), consider: frosted glass blur, subtle gradients, SF Symbols for state, smoother shape morphing, better color palette that works on both light/dark wallpapers. The toggle interaction should feel native, not like a jammed-in widget.
